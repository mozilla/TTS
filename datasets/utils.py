...
class TBPTT():
    ...
    def __init__(self, text, mel_spec, linear_spec, stop_target, lengths, tbp_len):
    ...
    def _setup_splits(self):
        ...
        self.stop_target = list(torch.split(self.stop_target, self.tbp_len, 1))
    ...
    def __next__(self):
        ...
        if self.num_iters > len(self.mel_spec):
            self.start = False
        return self.text, self.mel_spec.pop(), self.linear_spec.pop(),
               self.stop_target.pop(), self.lengths.pop()
