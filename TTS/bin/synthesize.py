...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Input file path. Will be ignored if --text is provided.",
    )
    parser.add_argument(
        "--out_path",
        type=str,
        default="tts_output.wav",
        help="Output wav file path. Defaults to tts_output.wav or the filename from the file given in --in_path with a .wav extension.",
    )
    ...
    # print the description if either text, in_path or list_models is not set
    if (args.text is None and args.in_path is None) and not args.list_models and not args.list_speaker_idxs:
        parser.parse_args(["-h"])
    ...
    # if we have an input file args.in_path, use its text instead of args.text
    if args.text is None:
        with open(args.in_path, 'r') as file:
            to_synth = file.read().replace('\n', '')
    else:
        to_synth = args.text

    # RUN THE SYNTHESIS
    print(" > Text: {}".format(to_synth))

    # kick it
    wav = synthesizer.tts(to_synth, args.speaker_idx, args.speaker_wav)

    # if we have an input file args.in_path, use it's filename with a .wav extension
    if args.in_path is None:
        to_output = args.out_path
    else:
        to_output = Path(args.in_path).stem+".wav"
    # save the results
    print(" > Saving output to {}".format(to_output))
    synthesizer.save_wav(wav, to_output)
